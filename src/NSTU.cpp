...
void processMat(const cv::Mat& image, int frameCount, TextDetection::TextDetector& textDetector, tesseract::TessBaseAPI& tessBaseApi, Tagging::Tagger& tagger)
{
...
void processImage(const std::string& inputFilePath, const std::string& outputFilePath)
{
    ...
    Tagging::Tagger tagger(inputFilePath, outputFilePath);
...
void processCamera(const std::string& outputFilePath)
{
    ...
    // Initialize tagger object.
    Tagging::Tagger tagger("camera", outputFilePath);
