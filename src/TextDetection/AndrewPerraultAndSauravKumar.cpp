...
void normalizeImage (IplImage * input, IplImage * output) {
    ...
    float maxVal = 0;
    float minVal = std::numeric_limits<float>::infinity();//1e100
...
//    cvShowImage("Gradient X", gradientX);
//    cvShowImage("Gradient Y", gradientY);
...
//    cvShowImage("Components", output3);
                      ...
                      std::vector<std::pair<Point2d,Point2d> > & compBB )
{
        ...
        for (std::vector<std::vector<Point2d> >::iterator it = components.begin(); it != components.end();it++) {
            ...
            float increment = 1.f/36.f;
            for (float theta = increment * PI; theta<PI/2.0f; theta += increment * PI) {
...
std::vector<Chain> makeChains( IplImage * colorImage,
    ...
    //std::cerr << std::endl;
    const float strictness = PI/6.0f;
    //merge chains
    int merges = 1;
    while (merges > 0) {
